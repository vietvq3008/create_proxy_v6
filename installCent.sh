...
function install_requred_packages(){
  yum update &>> $script_log_file

  requred_packages=("make" "g++" "wget" "curl" "cronie")
  local package
  for package in ${requred_packages[@]}; do
    if ! is_package_installed $package; then
      sudo yum install $package -y &>> $script_log_file
      if ! is_package_installed $package; then
...
function is_firewalld_running() {
  if systemctl status firewalld | grep -qw active; then
    return 0
  else
    return 1
  fi
...
function open_ufw_backconnect_ports(){
  ...
  if ! is_firewalld_running; then echo "Firewall not installed, ports for backconnect proxy opened successfully"; return; fi;

  if systemctl is-active firewalld >/dev/null 2>&1; then
    firewall-cmd --zone=public --add-port=$start_port-$last_port/tcp --permanent >> $script_log_file;
    firewall-cmd --zone=public --add-port=$start_port-$last_port/udp --permanent >> $script_log_file;
    firewall-cmd --reload

    if firewall-cmd --zone=public --list-ports | grep -qw $start_port-$last_port; then
  	echo "Firewalld ports for backconnect proxies opened successfully";
    else
        echo_log_err $(firewall-cmd --zone=public --list-ports);
        echo_log_err_and_exit "Cannot open ports for backconnect proxies, configure firewalld please";
    fi;


  else
    echo "Firewalld protection disabled, ports for backconnect proxy opened successfully";
  fi;
}
